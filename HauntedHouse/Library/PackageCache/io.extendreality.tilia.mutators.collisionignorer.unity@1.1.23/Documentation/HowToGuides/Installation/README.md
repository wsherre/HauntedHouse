...
The `Mutators.CollisionIgnorer` prefab manages the collision states between a collection of GameObjects in a [Unity] scene. This prefab can be included in a Unity software project via the [Unity Package Manager].
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.mutators.collisionignorer.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.mutators.collisionignorer.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Mutators CollisionIgnorer Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
