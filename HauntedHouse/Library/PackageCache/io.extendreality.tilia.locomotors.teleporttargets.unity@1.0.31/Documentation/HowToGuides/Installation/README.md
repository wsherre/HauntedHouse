...
The Teleport Targets provide two ways of determining a teleportable location:

* `Locomotors.TeleportTargets.Point` prefab denotes a specific point in the world that will form as the teleport location. This specific destination point can also be offset.
* `Locomotors.TeleportTargets.Area` prefab denotes an area of the world that is valid to teleport within.
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.locomotors.teleporttargets.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.locomotors.teleporttargets.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Locomotors TeleportTargets Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
