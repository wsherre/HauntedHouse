...
The `Trackers.PseudoBody` prefab provides the ability to track and represent a virtual body based on the location information of a physical body in a real world spatial location. This prefab can be included in a Unity software project via the [Unity Package Manager].
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.trackers.pseudobody.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.trackers.pseudobody.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Trackers PseudoBody Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
