...
The `Trackers.ColliderFollower` prefab allows a [Unity] GameObject with a Rigidbody and Collider to follow another GameObject around the scene by applying velocities to the Rigidbody so it moves within the physics system. This prefab can be included in a Unity software project via the [Unity Package Manager].
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.mutators.collisionignorer.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.trackers.colliderfollower.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Trackers ColliderFollower Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
